...
public class ChiTietHoaDonDao {
	...
	public boolean themChiTietHoaDon(ChiTietHoaDon ctHD) {
		try {
			...
			statement.setInt(9, ctHD.getMaGiamGia().getMaGiamGia());
	...
	public ArrayList<ChiTietHoaDon> getAllChiTietHoaDonByMaDonHang(int maDonHang) {
		...
		try {
			...
			while (rs.next()) {
				...
				Thuoc sp = new Thuoc(rs.getInt("MaThuoc"), rs.getString("TenThuoc"), null, "", "", "", LocalDate.now(),
						"", "", "", 0, 0, 0, 0, null);
				...
				ChiTietHoaDon ctHD = new ChiTietHoaDon(ma, soLuong, donViTinh, donGia, sp, thue, thanhTien, giamGia, hd,
						new MaGiamGia(maGiamGia));
				dsCTHD.add(ctHD);
			}
		} catch (Exception e) {
			e.printStackTrace();
